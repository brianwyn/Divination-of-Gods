...
public class Region {
	...
	public static void addObject(int x, int y, int height, int id) {
		int regionX = x >> 3;
		int regionY = y >> 3;
		int regionId = ((regionX / 8) << 8) + (regionY / 8);
		for (Region r : regions) {
			if (r.id() == regionId) {
				r.putObject(x, y, height, id);
				break;
			}
		}
	}
	...
	public static void addObject(int objectId, int x, int y, int height,
		...
		if (def.name != null || def.childrenIDs != null)
			addObject(x, y, height, objectId);
		int xLength;
		int yLength;
	...
	public static void addObject2(int x, int y, int height, int id) {
		if (height == 2) {
			height = 3;
		}
		int regionX = x >> 3;
	...
	public static boolean canAttack(Client a, Client b) {
		return canMove(a.getX(), a.getY(), b.getX(), b.getY(),
				a.heightLevel % 4, 1, 1);
	}
	...
	public static void load() {
		try {
			...
			System.out.println("[InsanityX.Net] - Region: Loaded maps!");
			System.out.println("[InsanityX.Net] InsanityX #1 RSPS Launched!");
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	public static boolean objectExists(int x, int y, int height, int id) {
		...
		for (Region r : regions) {
			if (r.id() == regionId) {
				for (StateObject o : r.objects) {
					if (o.getX() == x && o.getY() == y
							&& o.getHeight() == height && o.getType() == id) {
						return true;
					}
				}
			}
	...
	private List<StateObject> objects = new LinkedList<StateObject>();
	...
	private void putObject(int x, int y, int height, int id) {
		for (StateObject o : objects) {
			if (o.getX() == x && o.getY() == y && id == o.getType()) {
				return;
			}
		}
		objects.add(new StateObject(id, x, y, 0, height, 0, 0));
	}
