...
public class Smithing {
	...
	public static void smeltAdamant(final Client c) {
		if (c == null) {
			return;
		}
		if (c.playerLevel[Player.playerSmithing] < 70) {
			c.sendMessage("You need atleast level 70 to to make Adamant bars!");
			c.getPA().closeAllWindows();
			return;
		}
		if (!c.getItems().playerHasItem(449, 1)
				|| !c.getItems().playerHasItem(453, 6)) {
			c.sendMessage("You do not have the required ores: 1 Adamantite ore And 6 coal ores.");
			c.getPA().closeAllWindows();
			return;
		}
		c.isWalking = false;
	...
	public static void smeltBronze(final Client c) {
		...
		if (!c.getItems().playerHasItem(438, 1)
				|| !c.getItems().playerHasItem(436, 1)) {
			c.sendMessage("You do not have the required ores: 1 Tin and 1 Copper ore.");
			c.getPA().closeAllWindows();
			return;
		}
		c.isWalking = false;
		...
		CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				if (c.getItems().playerHasItem(438, 1)
					...
					c.turnPlayerTo(c.objectX, c.objectY);
					c.startAnimation(896);
					c.getItems().deleteItem(438, c.getItems().getItemSlot(438),
							1);
					c.getItems().deleteItem(436, c.getItems().getItemSlot(436),
							1);
	...
	public static void smeltGold(final Client c) {
		...
		if (c.playerLevel[Player.playerSmithing] < 40) {
			c.sendMessage("You need atleast level 40 to to make Gold bars!");
			c.getPA().closeAllWindows();
			return;
		}
		if (!c.getItems().playerHasItem(444, 1)) {
			c.sendMessage("You do not have the required ore: 1 Gold ore");
			c.getPA().closeAllWindows();
			return;
		}
		c.isWalking = false;
		...
		CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				if (c.getItems().playerHasItem(444, 1)) {
					c.turnPlayerTo(c.objectX, c.objectY);
					c.startAnimation(896);
					c.getItems().deleteItem(444, c.getItems().getItemSlot(444),
							1);
					c.getItems().addItem(2357, 1);
					c.getPA().addSkillXP(1300, Player.playerSmithing);
					// c.spinning = true;
					// container.stop();
				}
			}
	...
	public static void smeltIron(final Client c) {
		...
		if (!c.getItems().playerHasItem(440, 1)) {
			c.sendMessage("You do not have the required ore: 1 Iron ore.");
			c.getPA().closeAllWindows();
			return;
		}
		if (c.playerLevel[Player.playerSmithing] < 15) {
			c.sendMessage("You need atleast level 15 to to make Iron bars!");
			c.getPA().closeAllWindows();
			return;
		}
		c.isWalking = false;
		...
		CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				if (c.getItems().playerHasItem(440, 1)) {
					...
					c.getPA().addSkillXP(550, Player.playerSmithing);
					// c.spinning = true;
					// container.stop();
				}
			}
	...
	public static void smeltMithril(final Client c) {
		...
		if (c.playerLevel[Player.playerSmithing] < 50) {
			c.sendMessage("You need atleast level 50 to to make Mithril bars!");
			c.getPA().closeAllWindows();
			return;
		}
		if (!c.getItems().playerHasItem(447, 1)
				|| !c.getItems().playerHasItem(453, 4)) {
			c.sendMessage("You do not have the required ores: 1 Mithril ore And 4 coal ores.");
			c.getPA().closeAllWindows();
			return;
		}
		c.isWalking = false;
	...
	public static void smeltRune(final Client c) {
		...
		if (c.playerLevel[Player.playerSmithing] < 85) {
			c.sendMessage("You need atleast level 85 to to make Runite bars!");
			c.getPA().closeAllWindows();
			return;
		}
		if (!c.getItems().playerHasItem(451, 1)
				|| !c.getItems().playerHasItem(453, 8)) {
			c.sendMessage("You do not have the required ores: 1 Runite ore And 8 coal ores.");
			c.getPA().closeAllWindows();
			return;
		}
		c.isWalking = false;
	...
	public static void smeltSilver(final Client c) {
		...
		if (c.playerLevel[Player.playerSmithing] < 20) {
			c.sendMessage("You need atleast level 20 to to make Silver bars!");
			c.getPA().closeAllWindows();
			return;
		}
		if (!c.getItems().playerHasItem(442, 1)) {
			c.sendMessage("You do not have the required ore: 1 Silver ore");
			c.getPA().closeAllWindows();
			return;
		}
		c.isWalking = false;
		...
		CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				if (c.getItems().playerHasItem(442, 1)) {
					c.turnPlayerTo(c.objectX, c.objectY);
					c.startAnimation(896);
					c.getItems().deleteItem(442, c.getItems().getItemSlot(442),
							1);
					c.getItems().addItem(2355, 1);
					c.getPA().addSkillXP(700, Player.playerSmithing);
					// c.spinning = true;
					// container.stop();
				}
			}
	...
	public static void smeltSteel(final Client c) {
		...
		if (c.playerLevel[Player.playerSmithing] < 30) {
			c.sendMessage("You need atleast level 30 to to make Steel bars!");
			c.getPA().closeAllWindows();
			return;
		}
		if (!c.getItems().playerHasItem(440, 1)
				|| !c.getItems().playerHasItem(453, 2)) {
			c.sendMessage("You do not have the required ores: 1 Iron ore & 2 Coal ores.");
			c.getPA().closeAllWindows();
			return;
		}
		c.isWalking = false;
		...
		CycleEventHandler.getSingleton().addEvent(c, new CycleEvent() {
			...
			public void execute(CycleEventContainer container) {
				...
				if (!c.getItems().playerHasItem(440, 1)
						|| !c.getItems().playerHasItem(453, 2)) {
					c.sendMessage("You do not have the required ore: 1 Iron ore & 2 Coal ores.");
					c.getPA().closeAllWindows();
					container.stop();
					return;
				}
	...
	public boolean doaction(Client c, int toadd, int toremove, int toremove2,
		...
		if (c.getItems().playerHasItem(toremove, toremove2)) {
			...
			if (toadd == 1205 && !c.task1[3]) {
				c.task1[3] = true;
				c.sendMessage("You've completed the task: Smith a Bronze Dagger!");
				c.TPoints += 1;
				if (c.TPoints == 1) {
				...
				c.achievementInterface("Smith a Bronze Dagger!");
			}
			c.smithM += 1;
			if (c.smithM >= 150 && !c.task3[3]) {
				GabbesAchievements.handleEliteTask(c, 3, 3,
						"Smith 150 Runite daggers!");
			}
			if (maketimes > 1
	...
	public void sendSmelting() { // Creds to Sanity for this 1
