 ...
 * Herblore.java
 * 
 * @author Eli (Spetsnaz) credits to Sanity
 **/
...
public class Herblore {
	...
	public static final int[] HERBS = { 249, 251, 253, 255, 257, 259, 261, 263,
			265, 267, 2481, 2998, 3000 };

	private static int[][] herbClean = { { 199, 249, 1, 3 },
	...
	private static int[][] makePotion = { { 249, 91, 121, 1, 25, 221 },
			{ 251, 93, 175, 5, 38, 235 }, { 253, 95, 115, 12, 50, 225 },
			{ 255, 97, 127, 22, 63, 223 }, { 257, 99, 139, 38, 88, 231 },
			{ 259, 101, 145, 45, 100, 221 }, { 261, 103, 2448, 40, 100, 235 },
			{ 263, 105, 157, 55, 125, 225 }, { 265, 107, 163, 66, 155, 239 },
			{ 267, 109, 169, 72, 163, 245 }, { 269, 111, 189, 78, 175, 247 },
			{ 2481, 2483, 2454, 69, 158, 4621 },
			{ 2998, 3002, 6687, 81, 180, 6693 },
			{ 3000, 3004, 3026, 63, 143, 223 } };

	public static int rHerb() {
	...
	public void finishPot(int itemUsed, int usedWith, int slot) {
		if (c.playerLevel[Player.playerHerblore] >= makePotion[slot][3]) {
			if (c.getItems().playerHasItem(itemUsed)
					&& c.getItems().playerHasItem(usedWith)) {
				c.getItems().deleteItem(itemUsed,
						c.getItems().getItemSlot(itemUsed), 1);
				c.getItems().deleteItem(usedWith,
			...
			c.sendMessage("You need a herblore level of " + makePotion[slot][3]
					+ " to make this potion.");
		}
	}
	...
	private void idHerb(int slot) {
		if (c.getItems().playerHasItem(herbClean[slot][0])) {
			if (c.playerLevel[Player.playerHerblore] >= herbClean[slot][2]) {
				c.getItems().deleteItem(herbClean[slot][0],
						c.getItems().getItemSlot(herbClean[slot][0]), 1);
				c.getItems().addItem(herbClean[slot][1], 1);
				c.getPA().addSkillXP(
						herbClean[slot][3] * Config.HERBLORE_EXPERIENCE,
						Player.playerHerblore);
				// c.sendMessage("You identify the herb as a " +
				// c.getItems().getItemName(herbClean[slot][1]) + "."); // If
				// you want to show the name
				c.sendMessage("You clean the dirt off the herb.");
			} else {
				c.sendMessage("You need a herblore level of "
						+ herbClean[slot][2] + " to clean this herb.");
			}
		}
	}
	...
	public void makeUnfPot(int herbId) {
		if (c.getItems().playerHasItem(vial)
			...
			if (c.playerLevel[Player.playerHerblore] >= makePotion[slot][3]) {
				...
				c.sendMessage("You need a herblore level of "
						+ makePotion[slot][3] + " to make this potion.");
			}
		} else {
