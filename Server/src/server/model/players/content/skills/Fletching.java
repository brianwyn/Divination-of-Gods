	...
	public void fletchBow(int index) {
		...
		for (int j = 0; j < amount; j++) {
			if (c.getItems().playerHasItem(logType[index], 1)) {
				if (c.playerLevel[Player.playerFletching] >= reqs[index]
						|| fletchType == 3) {
					c.getItems().deleteItem(logType[index],
							c.getItems().getItemSlot(logType[index]), 1);
					c.getItems().addItem(toAdd, amountToAdd);
					c.getPA().closeAllWindows();
					c.getPA().addSkillXP(
							getExp(index) * Config.FLETCHING_EXPERIENCE,
							Player.playerFletching);
				} else {
					c.sendMessage("You need a fletching level of "
							+ reqs[index] + " to fletch this item.");
					break;
				}
			} else {
				break;
		...
		for (int j = 0; j < arrows.length; j++) {
			if ((item1 == arrows[j][0] && item2 == arrows[j][1])
				...
				if (c.getItems().playerHasItem(item1, 15)
					...
					if (c.playerLevel[Player.playerFletching] >= arrows[j][4]) {
