...
public class Woodcutting extends SkillHandler {
	...
	public static boolean gabbesquickrespawnfix = false;

	private static int[][] axes = { { 1351, 1, 879, 10 }, { 1349, 1, 877, 10 },
			{ 1353, 6, 875, 10 }, { 1361, 6, 873, 9 }, { 1355, 21, 871, 8 },
			{ 1357, 31, 869, 7 }, { 1359, 41, 867, 5 }, { 6739, 61, 2846, 4 },
			{ 13661, 80, 10251, 4 }, };
	...
	public static void attemptData(final Client c, final int tree,
		...
		if (c.CanNotWoodcut()) {
			c.sendMessage("You cannot cut wood here.");
			c.playerIsWoodcutting = false;
			return;
		}
		...
		if (!hasRequiredLevel(c, 8, c.playerSkillProp[8][2], "woodcutting",
				"cut this tree")) {
			resetWoodcutting(c);
			c.playerIsWoodcutting = false;
			return;
		...
		for (int i = 0; i < trees.length; i++) {
			for (int l = 0; l < 6; l++)
				if (tree == trees[l][i]) {
					CycleEventHandler.getSingleton().stopEvents(c, 424); // let's
																			// stop
																			// the
																			// event
																			// first.
					CycleEventHandler.getSingleton().addEvent(424, c,
							new CycleEvent() {
								public void execute(CycleEventContainer e) {
									...
									if (random2 == 3
											&& c.playerEquipment[Player.playerWeapon] == 13661
											&& c.playerSkillProp[8][6] > 0) {
										handleAdze(c);
									} else {
					...
					CycleEventHandler.getSingleton().stopEvents(c, 425); // let's
																			// stop
																			// the
																			// event
																			// first.
					CycleEventHandler.getSingleton().addEvent(425, c,
							new CycleEvent() {
								public void execute(CycleEventContainer e) {
									if (c.disconnected) {
										e.stop();
	...
	private static boolean canUseAxe(Client c) {
		...
		if (performCheck(c, 1349, 1) || performCheck(c, 1351, 1)
				...
				|| performCheck(c, 13661, 80)) {
			axe = true;
		}
		return axe;
	}

	private static void createStump(final Client c, final int i, final int obX,
			final int obY) {
		if (c == null) {
			return;
		}
		if (!c.task1[4]) {
			c.task1[4] = true;
			c.sendMessage("You've completed the task: Cut down a tree!");
			c.TPoints += 1;
			if (c.TPoints == 1) {
			...
			c.achievementInterface("Cut down a tree!");
		}
	...
	public static void handleAdze(Client c) {
		c.gfx0(1776);
				...
				if (c.getY() > 0) {
					if (c.getId() > 0) {
						Server.itemHandler.createGroundItem(c, 5070, c.getX(),
								c.getY(), 1, c.getId());
						c.sendMessage("A bird's nest falls down from the tree!");
						c.gfx0(1778);
					}
				}
			}
