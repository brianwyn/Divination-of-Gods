...
public class Hunter {
	...
	public void catchButterFly(String npcName, int Net, int npcId, int itemId,
		...
		if (System.currentTimeMillis() - c.foodDelay > 1500) { // anti spamm
			if (c.playerLevel[21] >= Req) { // first we check if he's high
											// enough to catch
				if (HasNet()) {
				...
				c.sendMessage("You need at least " + Req
	...
	public void catchImpling(String npcName, int Net, int npcId, int itemId,
			int AmtExp, int Req, int playerId) {
		// npcName = Server.npcHandler.getNpcListName(npcId);
		if (System.currentTimeMillis() - c.foodDelay >= 1500) {
			if (c.playerLevel[21] >= Req) {
				...
				c.sendMessage("You need atleast " + Req
	...
	public boolean HasNet() {
		if (c.playerEquipment[Player.playerWeapon] == 10010
				|| c.playerEquipment[Player.playerWeapon] == 11259) {
			return true;
		} else {
			return false;
		}
