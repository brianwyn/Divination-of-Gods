...
public class Potions {
	...
	public static void enchanceStat3(int skillID, boolean sup, Client c) {
		c.playerLevel[skillID] += getBoostedStat2(skillID, sup, c);
		c.getPA().refreshSkill(skillID);
	}

	public static int getBoostedStat2(int skill, boolean sup, Client c) {
		int increaseBy = 0;
		if (sup)
	...
	public void doOverload2(int itemId, int replaceItem, int slot) {
		c.sendMessage("These types of Overloads were disable in the latest update.");
		c.sendMessage("You will need to make new Overloads!");
		return;
	}
	...
	public void doTheBrew(int itemId, int replaceItem, int slot) {
		...
		if (c.hasOverloadBoost = true && c.playerLevel[2] > (int) (c
				.getLevelForXP(c.playerXP[3]) * 1.24)) {
			double ammount = c.torva() ? 1.22 : 1.17;
			c.startAnimation(829);
			c.playerItems[slot] = replaceItem + 1;
			c.getItems().resetItems(3214);
			c.playerLevel[3] += c.torva() ? getBrewStat(3, .21) : getBrewStat(
					3, .15);
	...
	public void drinkExtremePotion(int itemId, int replaceItem, int slot,
		...
		if (c.inWild()) {
			c.sendMessage("The effect of this extreme potion does not work in the wilderness.");
			return;
		}
	...
	public void drinkExtremePrayer(int itemId, int replaceItem, int slot,
			boolean rest) {
		c.startAnimation(829);
		c.playerItems[slot] = replaceItem + 1;
		c.getItems().resetItems(3214);
	...
	public void drinkPot(int itemId, int replaceItem, int slot) {
		if (c.inWild()) {
			c.sendMessage("The effects of the overload potion do not work in the wilderness.");
			return;
		}
		if (c.isDead && c.respawnTimer == -6) {
			return;
		}
		if (c.playerLevel[3] <= 50) {
			c.sendMessage("You need more than 50 hitpoints to survive the power of an overload.");
			return;
		}
		if (c.duelRule[6]) {
			c.sendMessage("Not alowed due to rule restrictions.");
			return;
		}
		c.overload = 600;
		c.getTimers().overload(c);
		c.overload();
		c.startAnimation(829);
		c.playerItems[slot] = replaceItem + 1;
		c.getItems().resetItems(3214);
	...
	public void handlePotion(int itemId, int slot) {
		...
		if (System.currentTimeMillis() - c.potDelay >= 1500) {
			...
			c.attackTimer++;
			c.sendMessage("You drink some of your "
			...
			case 155:
				...
				break;
			/*
			 * case 15332://overload 4 drinkPot(itemId, 15333, slot); break;
			 * case 15333://overload 3 drinkPot(itemId, 15334, slot); break;
			 * case 15334://overload 2 drinkPot(itemId, 15335, slot); break;
			 * case 15335://overload 1 drinkPot(itemId,229,slot); break; case
			 * 15332: doOverload2(itemId, 15333, slot); break; case 15333:
			 * doOverload2(itemId, 15334, slot); break; case 15334:
			 * doOverload2(itemId, 15335, slot); break; case 15335:
			 * doOverload2(itemId, 229, slot); break;
			 */
			case 6685: // brews
