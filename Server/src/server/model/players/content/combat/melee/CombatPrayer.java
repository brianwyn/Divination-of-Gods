...
public class CombatPrayer {
	...
	public static void activatePrayer(Client c, int i) {
		...
		if (c.duelRule[7]) {
			resetPrayers(c);
			c.sendMessage("Prayers have been disabled in this duel!");
			return;
		}
		...
		if (c.playerLevel[5] > 0 || !Config.PRAYER_POINTS_REQUIRED) {
			if (c.getPA().getLevelForXP(c.playerXP[5]) >= Player.PRAYER_LEVEL_REQUIRED[i]
				...
				case 18:
					if (System.currentTimeMillis() - c.stopPrayerDelay < 5000) {
						...
						c.getPA().sendFrame36(Player.PRAYER_GLOW[16], 0);
						c.getPA().sendFrame36(Player.PRAYER_GLOW[17], 0);
						c.getPA().sendFrame36(Player.PRAYER_GLOW[18], 0);
						return;
					}
					if (i == 16)
				...
				if (!headIcon) {
					if (c.prayerActive[i] == false) {
						...
						c.getPA().sendFrame36(Player.PRAYER_GLOW[i], 1);
					} else {
						c.prayerActive[i] = false;
						c.getPA().sendFrame36(Player.PRAYER_GLOW[i], 0);
					}
				} else {
					if (c.prayerActive[i] == false) {
						c.prayerActive[i] = true;
						c.getPA().sendFrame36(Player.PRAYER_GLOW[i], 1);
						c.headIcon = Player.PRAYER_HEAD_ICONS[i];
				...
				c.getPA().sendFrame126(
						"You need a @blu@Prayer level of "
								+ Player.PRAYER_LEVEL_REQUIRED[i] + " to use "
