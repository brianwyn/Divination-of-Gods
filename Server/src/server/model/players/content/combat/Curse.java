...
public class Curse {
	...
	public void activateCurse(int i) {
		if (c.altarPrayed == 0) {
			System.out.println("" + c.playerName + " just tried prayer bug!");
			c.sendMessage("Nice try fucker, reported to gabbe.");
			return;
		}

		if (c.inDominionTower()) {
			c.getPA().sendFrame36(Player.CURSE_GLOW[i], 0);
			c.sendMessage("Gabbe doesn't allow warriors to use prayers!");
			return;
		}
		if (c.duelRule[7]) {
			resetCurse();
			c.sendMessage("Curses have been disabled in this duel!");
			return;
		}
		...
		// 10 = leechAtk//11 = leechRng//12 = leechMge//13 = leechDef//14 =
		// leechStr
		// 15 = leechEnrgy//16 = leechSpec//17 = wrath//18 = soul//19 = turmoil

		if (c.playerLevel[5] > 0 || !Config.PRAYER_POINTS_REQUIRED) {
			if (c.getPA().getLevelForXP(c.playerXP[5]) >= Player.CURSE_LEVEL_REQUIRED[i]
				...
				case 9:
					if (System.currentTimeMillis() - c.stopPrayerDelay < 5000) {
						...
						c.getPA().sendFrame36(Player.CURSE_GLOW[7], 0);
						c.getPA().sendFrame36(Player.CURSE_GLOW[8], 0);
						c.getPA().sendFrame36(Player.CURSE_GLOW[9], 0);
						return;
					}
					if (i == 7)
				...
				if (!headIcon) {
					if (c.curseActive[i] == false) {
						...
						c.getPA().sendFrame36(Player.CURSE_GLOW[i], 1);
					} else {
					...
					if (c.curseActive[i] == false) {
						c.curseActive[i] = true;
						c.getPA().sendFrame36(Player.CURSE_GLOW[i], 1);
						c.headIcon = Player.CURSE_HEAD_ICONS[i];
				...
				c.getPA().sendFrame126(
						"You need a @blu@Prayer level of "
								+ Player.CURSE_LEVEL_REQUIRED[i] + " to use "
								+ Player.CURSE_NAME[i] + ".", 357);
	...
	public void appendDeflect(int otherPlayer, int damage, int icon) {// Handles
																		// Deflect
																		// Prayers
	...
	public void atkCurse(int i) {
		for (int j = 0; j < atk.length; j++) {
			if (atk[j] != i) {
				...
				c.getPA().sendFrame36(Player.CURSE_GLOW[atk[j]], 0);
			}
		}
	}
	...
	public void defCurse(int i) {
		for (int j = 0; j < def.length; j++) {
			if (def[j] != i) {
				...
				c.getPA().sendFrame36(Player.CURSE_GLOW[def[j]], 0);
			}
		}
	}
	...
	public void mgeCurse(int i) {
		for (int j = 0; j < mge.length; j++) {
			if (mge[j] != i) {
				...
				c.getPA().sendFrame36(Player.CURSE_GLOW[mge[j]], 0);
			}
		}
	}
	...
	public void rngCurse(int i) {
		for (int j = 0; j < rng.length; j++) {
			if (rng[j] != i) {
				...
				c.getPA().sendFrame36(Player.CURSE_GLOW[rng[j]], 0);
			}
		}
	}
	...
	public void sprtCurse(int i) {
		for (int j = 0; j < sprt.length; j++) {
			if (sprt[j] != i) {
				...
				c.getPA().sendFrame36(Player.CURSE_GLOW[sprt[j]], 0);
			}
		}
	}
	...
	public void strCurse(int i) {
		for (int j = 0; j < str.length; j++) {
			if (str[j] != i) {
				...
				c.getPA().sendFrame36(Player.CURSE_GLOW[str[j]], 0);
			}
		}
	}
