...
public class RangeMaxHit extends RangeData {
	...
	public static int calculateRangeAttack(Client c) {
		...
		if (c.fullVoidRange()) {
			rangeLevel += c.getLevelForXP(c.playerXP[Player.playerRanged]) * 0.1;
		}
		if (c.prayerActive[3] || c.curseActive[11]) {
			rangeLevel *= 1.05;
	...
	public static int calculateRangeDefence(Client c) {
		...
		if (c.prayerActive[0]) {
			...
			defenceLevel += c.getLevelForXP(c.playerXP[Player.playerDefence])
					* 0.20 + c.getdef;
		}
	...
	public static int maxHit(Client c) {
		...
		int d = getRangeStr(c.usingBow ? c.playerEquipment[Player.playerArrows]
