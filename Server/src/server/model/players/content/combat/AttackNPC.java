...
public class AttackNPC {
	...
	public static void applyNpcMeleeDamage(Client c, int i, int damageMask,
		...
		if (NPCHandler.npcs[i] != null) {
			if (NPCHandler.isHunterIMP(NPCHandler.npcs[i].npcType)) {

				if (c.playerMagicBook != 0) {
					c.getCombat().resetPlayerAttack();
					return;
				}
				if (c.spellID != 1592 && c.spellID != 1572 && c.spellID != 1582) {
					c.getCombat().resetPlayerAttack();
					return;
				}
				if (c.spellID == 1592 || c.spellID == 1572 || c.spellID == 1582
					...
					if (c.spellID == 1572) {
						...
						Server.npcHandler.freezeIMP(c, i, 10, c.huntIMPX,
								c.huntIMPY);
					} else if (c.spellID == 1582) {
						Server.npcHandler.freezeIMP(c, i, 17, c.huntIMPX,
								c.huntIMPY);
					} else if (c.spellID == 1592) {
						Server.npcHandler.freezeIMP(c, i, 25, c.huntIMPX,
								c.huntIMPY);
					}
					return;
				}
		...
		if (c.curseActive[10]) {
			if (Misc.random(11) == 2) {
				if (getID(c) == -1)
					return;
				LeechNPC.leechNPC(c, 0);
			}
		}
		if (c.curseActive[18])
			LeechNPC.applySoulSplit(c);

		if (c.curseActive[11]) {
			if (Misc.random(11) == 2) {
				if (getID(c) == -1)
					return;
				LeechNPC.leechNPC(c, 4);
			}
		}
	...
	public static void attackNpc(Client c, int i) {
		if (NPCHandler.npcs[i] != null) {
			...
			if (c.playerEquipment[Player.playerWeapon] == 20012
					&& c.playerLevel[6] > 80 && c.spellId == 0
					&& !c.autocasting) {
				c.autocasting = true;
				c.spellId = 52;
				c.autocastId = 52;
			...
			if ((c.underAttackBy > 0 || c.underAttackBy2 > 0)
					&& c.underAttackBy2 != i && !c.inMulti() && !c.inBarbDef) {
				c.getCombat().resetPlayerAttack();
			...
			if (NPCHandler.npcs[i].spawnedBy == c.getId()
					&& c.hasFollower == NPCHandler.npcs[i].npcType) {
				c.sendMessage("You cannot attack your own familiar!");
				c.getCombat().resetPlayerAttack();
				return;
			}
			if (NPCHandler.npcs[i].inMulti() && c.hasFollower > 0
					&& !c.InDung() && !c.arenas()) {
			...
			if (c.attackTimer <= 0) {
				...
				if (c.getCombat().correctBowAndArrows() < c.playerEquipment[Player.playerArrows]
						...
						&& c.playerEquipment[Player.playerWeapon] != 9185
						&& c.playerEquipment[Player.playerWeapon] != 18357
						&& c.playerEquipment[Player.playerWeapon] != 15241) {
				...
				if (c.usingBow && !c.usingOtherRangeWeapons && !c.usingMagic
					...
					if (c.playerEquipment[Player.playerWeapon] >= 4212
						...
						if (c.playerEquipment[3] == 11235
								&& c.playerEquipment[3] == 19146
								&& c.playerEquipment[3] == 19149
								&& c.playerEquipment[3] == 19143
								&& c.playerEquipment[3] == 14990) {
							c.getItems().deleteArrow();
						}
					}
			...
			if (!c.castingMagic && c.projectileStage > 0) { // range hit damage
				...
				if (c.playerEquipment[3] == 9185) {
					if (Misc.random(10) == 1) {
						if (damage > 0) {
							c.boltDamage = damage;
							c.getCombat().crossbowSpecial(c, i, true);
				...
				if (c.lastWeaponUsed == 19149 || c.bowSpecShot == 1)
					damage2 = Misc.random(c.getCombat().rangeMaxHit());
				if (c.playerEquipment[3] == 9185) {
					if (Misc.random(10) == 1) {
						if (damage > 0) {
				...
				if (c.lastWeaponUsed == 19143 || c.bowSpecShot == 1)
					damage2 = Misc.random(c.getCombat().rangeMaxHit());
				if (c.playerEquipment[3] == 9185) {
					if (Misc.random(10) == 1) {
						if (damage > 0) {
				...
				if (c.lastWeaponUsed == 11235 && c.lastWeaponUsed == 11235
						|| c.bowSpecShot == 1) {
					if (Misc.random(NPCHandler.npcs[i].defence) > Misc
							.random(10 + c.getCombat().calculateRangeAttack()))
						damage2 = 0;
