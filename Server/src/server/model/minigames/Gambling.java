...
public class Gambling {
	...
	public static int[][] randomPrize = {
			{ 4151, 1333, 4087, 4585, 6916, 6918, 6920, 6922, 6924, 6926, 7362,
					7364, 7366, 7368, 7370, 7372, 7374, 7376, 7378, 7380, 7382,
					7384, 7386, 7388, 7390, 7392, 7394, 7396, 7398, 7399, 7400,
					-1 },

			{ 1305, 1377, 1434, 2583, 2585, 2587, 2589, 2591, 2593, 2595, 2597,
					2599, 2601, 2603, 2605, 2607, 2609, 2611, 2613, 2615, 2617,
					2619, 2621, 2623, 2625, 2627, 2629, -1 },

			{ 2631, 2633, 2635, 2637, 2639, 2641, 2643, 2645, 2647, 2649, 2651,
					2653, 2655, 2657, 2659, 2661, 2663, 2665, 2667, 2669, 2671,
					2673, 2675, 10354, 3481, 3483, 3485, 3486, 3488, -1 },

			{ 1060, 1066, 1080, 1094, 1114, 1216, 1334, 1518, 2441, 2443, 2437,
					2435, 9244, 1713, 1616, 1320, 1713, 4151, 1333, 4087, 4585,
					6916, 6918, 6920, 6922, 6924, 6926, 7362, 7364, 7366, 7368,
					7370, 7372, 7374, 7376, 7378, 7380, 7382, 7384, 7386, 7388,
					7390, 7392, 7394, 7396, 7398, 7399, 7400, 1305, 1377, 1434,
					...
					2663, 2665, 2667, 2669, 2671, 2673, 2675, 10354, 3481,
					3483, 3485, 3486, 3488, -1 } };
	...
	public static void blackJack(Client c) {
		...
		if ((playerTotalHit > computerTotalHit)) {
			c.sendMessage("You win with " + playerTotalHit
					+ "! The gambler had " + computerTotalHit + ".");
			recieveGambleBet(c);
		} else if ((computerTotalHit > playerTotalHit)) {
			c.sendMessage("The gambler won with " + computerTotalHit
					+ ". You had " + playerTotalHit + ".");
		}
	...
	public static void playGame(final Client c) {
		...
		c.sendMessage("Your numbers are: [<col=255>" + number1
				+ "</col>] and [<col=255>" + number2 + "</col>].");
	...
	private static void recieveGambleBet(Client c) {
		if (hasBetEnough) {
			c.getItems().addItem(
					995,
					(blackJack ? (playerBet * 2) + (playerBet / 2)
							: (playerBet * 2)));
	...
	public static void rewardFive(Client c, boolean isDouble) {
		...
		if (chance >= 0 && chance <= 5) {
			addReward(c, 0, 32, 1,
					"Better luck next time! You win a low standard item.");
