...
/**
 * @author Brandyn - Redone by Gabbe, new event to handle the tokens added, it's
 *         more perfect.
 */
public class WarriorsGuild {
	...
	private final int[][] ARMORREQUIRED = {
			{ 1075, 1117, 1155, 4278, 20, 5, 30, 100 },
			{ 1067, 1115, 1153, 4279, 30, 7, 40, 100 },
			{ 1069, 1119, 1157, 4280, 40, 10, 60, 100 },
			{ 1077, 1125, 1165, 4281, 50, 15, 70, 100 },
			{ 1071, 1121, 1159, 4282, 60, 15, 80, 100 },
			{ 1091, 1123, 1161, 4283, 80, 17, 90, 10 },
			{ 1079, 1127, 1163, 4284, 100, 20, 100, 100 } };

	private final String[][] DEFENDER = { { "17273", "Flameburst" },
			{ "20072", "Dragon" }, { "8850", "Rune" }, { "8849", "Adamant" },
			{ "8848", "Mithril" }, { "8847", "Black" }, { "8846", "Steel" },
			{ "8845", "Iron" }, { "8844", "Bronze" } };
	 ...
	 * 
	 */
	private void animateArmor(Client c, int[] usedArmor, int npcId,
		...
		c.spawned = true;
		Server.npcHandler.spawnNpc(c, npcId, xCoord, yCoord, 0, 0, hitPoints,
				maxHit, attack, defence, true, true);
	}
			...
			for (int i = 0; i < ARMORREQUIRED.length; i++) {
				for (int f = 0; f < ARMORREQUIRED[0].length; f++) {
					if (itemId == ARMORREQUIRED[i][f]) {
						if (hasItem(c, ARMORREQUIRED[i][0], true)
								&& hasItem(c, ARMORREQUIRED[i][1], true)
								&& hasItem(c, ARMORREQUIRED[i][2], true)) {
							animateArmor(c, new int[] { ARMORREQUIRED[i][0],
									ARMORREQUIRED[i][1], ARMORREQUIRED[i][2] },
									ARMORREQUIRED[i][3], ARMORREQUIRED[i][4],
									ARMORREQUIRED[i][5], ARMORREQUIRED[i][6],
									ARMORREQUIRED[i][7]);
						}
					}
				}
	...
	private boolean hasItem(Client c, int itemId, boolean inventory) {
		return inventory ? c.getItems().playerHasItem(itemId) : c.getItems()
				.playerHasItem(itemId) || c.playerEquipment[5] == itemId;
	}
