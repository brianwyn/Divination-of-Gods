...
public static void transparentBox(int i, int j, int k, int l, int i1, int j1, int opac)
	...
	public static void createFadingBox(int x, int y, int w, int h, int c) {
		...
		for (int pX = x; pX < w + x; pX++) {
			...
			for (int pY = (h + y) - 1; pY > y; pY--) {
				...
				pixels[pX + pY * width] = ((k & 0xff00ff) * alphaIntensity
						+ (i3 & 0xff00ff) * alphaFuse & 0xff00ff00)
						+ ((k & 0xff00) * alphaIntensity + (i3 & 0xff00)
								* alphaFuse & 0xff0000) >> 8;
			}
	...
	public static void createFadingLine(int x, int y, int colour, int w) {
		...
		for (int pX = x; pX < w + x; pX++) {
			...
			pixels[pX + y * width] = ((k & 0xff00ff) * alphaIntensity
					+ (i3 & 0xff00ff) * alphaFuse & 0xff00ff00)
					+ ((k & 0xff00) * alphaIntensity + (i3 & 0xff00)
							* alphaFuse & 0xff0000) >> 8;
		}
