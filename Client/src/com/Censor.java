...
final class Censor {
	...
	private static int method503(char ac[], char ac1[], int j) {
		if (j == 0)
			return 2;
		for (int k = j - 1; k >= 0 && method517(ac[k]); k--)
			if (ac[k] == '@')
				return 3;
		...
		if (l >= 3)
			...
			return method517(ac[j - 1]) ? 1 : 0;
	}

	private static int method504(char ac[], int i, char ac1[]) {
		if (i + 1 == ac1.length)
			return 2;
		for (int j = i + 1; j < ac1.length && method517(ac1[j]); j++)
			if (ac1[j] == '.' || ac1[j] == ',')
				return 3;
	...
	private static int method508(char ac[], char ac1[], int i) {
		if (i + 1 == ac.length)
			return 2;
		for (int j = i + 1; j < ac.length && method517(ac[j]); j++)
			if (ac[j] == '\\' || ac[j] == '/')
				return 3;
			...
			if (!flag1) {
				...
				if (!flag5 || !flag6) {
					...
					for (; !flag7 && k2 < l; k2++) {
						...
						for (j3 = 0; j3 < 3
								&& k2 + j3 < ac.length
								&& (!method517(ac[k2 + j3]) || ac[k2 + j3] == '\''); j3++)
							ac2[j3] = ac[k2 + j3];
		...
		if (c2 >= 'a' && c2 <= 'm') {
			...
			if (c2 == 'b')
				if (c1 == '6' || c1 == '8')
					...
					return c1 == '1' && c == '3' || c1 == 'i' && c == '3' ? 2
							: 0;
			if (c2 == 'c')
				return c1 == '(' || c1 == '<' || c1 == '{' || c1 == '[' ? 1 : 0;
			if (c2 == 'd')
				return c1 == '[' && c == ')' || c1 == 'i' && c == ')' ? 2 : 0;
			if (c2 == 'e')
				return c1 == '3' || c1 == '\u20AC' ? 1 : 0;
			if (c2 == 'f')
				if (c1 == 'p' && c == 'h')
